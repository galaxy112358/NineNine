...
class NineNinePlayerOL :public NineNineAgent
{
public:
	int winRate;
	int careerTotal;
	NineNinePlayerOL(string N = "PlayerOL", string SN = " ", int CN = -1, int S = 0, int T = 0);
	POINT decision(const int(*board)[9], const int* occupy, const int* available);
private:
	void chooseName();
	void chooseSkin();
...
class NineNineOpponentOL:public NineNineAgent
{
public:
	SOCKET serverSocket;
	int uniNum;
	int winRate;
	int careerTotal;
	NineNineOpponentOL(SOCKET sS, string N = "OpponentOL", string SN = " ", int CN = -1, int S = 0, int T = 0, int wR = 0, int cT = 0, int uniN = 0);
	POINT decision(const int(*board)[9], const int* occupy, const int* available);
private:
	void chooseName();
	void chooseSkin();
...
public:
	...
	bool sendPlayerInfo(NineNinePlayerOL* player);
	int getUniN();
	bool getPlayerInfo(NineNineAgent*& opponent);
};